import os
from typing import List, Dict

class FakeAVCelebDataset:
    def __init__(self, root_dir: str):
        self.root_dir = root_dir
        self.splits = {
            'real': os.path.join(root_dir, 'real'),
            'fake': os.path.join(root_dir, 'fake'),
        }
        self.error_log = os.path.join(self.root_dir, 'fakeavceleb_errors.log')

    def log_error(self, msg):
        with open(self.error_log, 'a') as f:
            f.write(msg + '\n')

    def get_split(self, split: str) -> List[Dict]:
        assert split in self.splits, f"Unknown split: {split}"
        data_dir = self.splits[split]
        entries = []
        if not os.path.exists(data_dir):
            return entries
        try:
            actor_ids = os.listdir(data_dir)
        except Exception as e:
            self.log_error(f"[ERROR] Listing {data_dir}: {e}")
            return entries
        for actor_id in actor_ids:
            actor_dir = os.path.join(data_dir, actor_id)
            if not os.path.isdir(actor_dir):
                continue
            try:
                files = os.listdir(actor_dir)
            except Exception as e:
                self.log_error(f"[ERROR] Listing {actor_dir}: {e}")
                continue
            for fname in files:
                try:
                    if fname.endswith('.mp4'):
                        entries.append({
                            'file_path': os.path.join(actor_dir, fname),
                            'label': split,  # 'real' or 'fake'
                            'actor_id': actor_id
                        })
                except Exception as e:
                    self.log_error(f"[ERROR] File {fname} in {actor_dir}: {e}")
        return entries

    def get_all(self) -> List[Dict]:
        all_entries = []
        for split in self.splits:
            all_entries.extend(self.get_split(split))
        return all_entries 